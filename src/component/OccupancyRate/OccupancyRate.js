      ...
      const fetchData=async()=>{
        try{
          if(city === 'out') {
             ...
             const data=response.data.rechart_friendly.month_data_ocu_ls;
             setData(data);
             console.log(response.data)
             const value= response.data.calculated.month_data_ocu;
             const occData= Math.ceil ((value.apr_ocu + value.aug_ocu+value.dec_ocu + value.jan_ocu+value.jul_ocu + value.jun_ocu+value.mar_ocu + value.may_ocu+value.nov_ocu + value.oct_ocu+value.sep_ocu) / 12);
